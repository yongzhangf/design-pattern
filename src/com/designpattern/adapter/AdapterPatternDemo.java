...
import com.designpattern.adapter.entity.*;
...
public class AdapterPatternDemo {
     ...
     * 笔记本通过读卡去读取TF卡
     * 一、先模拟计算机读取SD卡
     * 二、在不改变计算机读取SD卡接口的情况下，通过适配器模式读取TF卡
     */
    public static void main(String[] args) {

        SDCard sdCard = new KingstonSD();

        ThinkPad thinkPad = new ThinkPad();
        thinkPad.connectSD(sdCard);

        String sdCardMsg = thinkPad.readSD();
        System.out.println(sdCardMsg);

        thinkPad.writeSD("From ThinkPad!");

        TFCard tfCard = new SandiskTF();
        thinkPad.connectSD(new TFAdapter(tfCard));

        sdCardMsg = thinkPad.readSD();
        System.out.println(sdCardMsg);
